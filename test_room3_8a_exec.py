"""8A (1M) leftover loud loc-strong cooled window / 12.5% pack / 3.5% stop floor."""
...
def _8a_slices():
    """Session up ≥5%, last 5-bar vel in −2–<2, last green ≥2% loc ≥0.60, loud RVOL."""
    rows = []
    px = 1.00
    for _ in range(6):
    ...
    last = _bar(px, px * 1.022, px * 0.999, px * 1.016, v=400)
    rows.append(last)
    return rows, float(last["c"])


def test_8a_wallpaper_does_not_gene():
    quiet = [_bar(1.0, 1.002, 0.999, 1.001, v=80)] * 10
    assert m._8a_gene_ok(quiet) is False


def test_8a_gene_not_pop_or_7a():
    slices, _px = _8a_slices()
    assert m._8a_gene_ok(slices) is True
...
def test_8a_fill_now_after_open():
...
def test_8a_first_of_day_blocks():
    ss = _SS(_now_et=datetime(2026, 9, 11, 13, 0, tzinfo=ET))
    m._8a_mark_used(ss, "FAMI")
    slices, px = _8a_slices()
    ready, note = m._entry_trigger_ready(
        {"ticker": "FAMI"},
        slices,
...
def test_8a_pack_target_12_5pct():
    lot = {
        "strategy": "8A (1M)",
        "tf": "1m",
        "letter": "8A (1M)",
        "exit_style": m.EIGHT_A_EXIT_STYLE,
        "entry_px": 1.00,
        "exit_stop_px": 0.965,
        "exit_tgt_px": 1.125,
...
def test_append_lot_stamps_8a_pack_exit():
    ss = _SS()
    ss.room3_lots = []
