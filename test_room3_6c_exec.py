"""6C (1M) milder dump bounce than 6B / 14% pack / 3.5% stop floor."""
...
def _6c_slices():
    """Vel5 about −0.8 (not 6B’s ≤−2.5), last green ≥2%, under VWAP."""
    rows = [_bar(1.05, 1.052, 1.048, 1.05, v=40)] * 8
    last = _bar(1.038, 1.055, 1.028, 1.042, v=42)
    rows.append(last)
    return rows, float(last["c"])


def test_6c_wallpaper_does_not_gene():
    quiet = [_bar(1.0, 1.002, 0.999, 1.001, v=80)] * 10
    assert m._6c_gene_ok(quiet) is False


def test_6c_gene_under_vwap_not_6a_or_6b():
    slices, _px = _6c_slices()
    assert m._6c_gene_ok(slices) is True
    assert m._6a_gene_ok(slices) is False
    assert m._6b_gene_ok(slices) is False
    assert m._5a_gene_ok(slices) is False


def test_6c_fill_now_after_open():
    ss = _SS(_now_et=datetime(2026, 9, 11, 12, 0, tzinfo=ET))
    slices, px = _6c_slices()
...
def test_6c_first_of_day_blocks():
    ss = _SS(_now_et=datetime(2026, 9, 11, 13, 0, tzinfo=ET))
    m._6c_mark_used(ss, "FAMI")
    slices, px = _6c_slices()
    ready, note = m._entry_trigger_ready(
        {"ticker": "FAMI"},
        slices,
...
def test_6c_pack_target_14pct():
    lot = {
        "strategy": "6C (1M)",
        "tf": "1m",
        "letter": "6C (1M)",
        "exit_style": m.SIX_C_EXIT_STYLE,
        "entry_px": 1.00,
        "exit_stop_px": 0.965,
        "exit_tgt_px": 1.14,
    ...
    why = m._lot_should_exit(
        ...
        last_px=1.15,
        patience=True,
        bar={"h": 1.15, "l": 1.05, "c": 1.15},
        session_state=ss,
    )
    assert why.startswith("target")


def test_append_lot_stamps_6c_pack_exit():
    ss = _SS()
    ss.room3_lots = []
