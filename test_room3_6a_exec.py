"""6A (1M) down-window bounce under VWAP / 16% pack / 3.5% stop floor."""
...
def _6a_slices():
    """Dump ~1.6% bars so the 5-bar window stays ≤−3% after the last green. Not 5A."""
    rows = [_bar(1.10, 1.101, 1.098, 1.099, v=40)]
    px = 1.099
    for _ in range(7):
        nxt = round(px * 0.984, 6)
...
def test_6a_wallpaper_does_not_gene():
    quiet = [_bar(1.0, 1.002, 0.999, 1.001, v=80)] * 10
    assert m._6a_gene_ok(quiet) is False


def test_6a_gene_under_vwap_not_5a():
    slices, _px = _6a_slices()
    assert m._6a_gene_ok(slices) is True
    assert m._5a_gene_ok(slices) is False


def test_6a_fill_now_after_open():
...
def test_6a_first_of_day_blocks():
    ss = _SS(_now_et=datetime(2026, 9, 11, 13, 0, tzinfo=ET))
    m._6a_mark_used(ss, "FAMI")
    slices, px = _6a_slices()
    ready, note = m._entry_trigger_ready(
        {"ticker": "FAMI"},
        slices,
...
def test_6a_pack_target_16pct():
    lot = {
        "strategy": "6A (1M)",
        "tf": "1m",
        "letter": "6A (1M)",
        "exit_style": m.SIX_A_EXIT_STYLE,
        "entry_px": 1.00,
        "exit_stop_px": 0.965,
        "exit_tgt_px": 1.16,
...
def test_append_lot_stamps_6a_pack_exit():
    ss = _SS()
    ss.room3_lots = []
    row = room3_engine.lots.append_lot(
        ss,
        {
            ...
            "qty": 10,
            "entry_px": 1.00,
            "structural_move_pct": 28.0,
            "exit_style": m.SIX_A_EXIT_STYLE,
            "exit_stop_px": 0.965,
            "exit_tgt_px": 1.16,
        },
    )
    assert row["exit_style"] == m.SIX_A_EXIT_STYLE
    assert abs(row["exit_tgt_px"] - 1.16) < 1e-6
    assert abs(float(row["exit_r_frac"]) - 0.035) < 1e-9
    assert ss.room3_6a_used_day["FAMI"]
